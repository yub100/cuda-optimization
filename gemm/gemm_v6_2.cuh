...
__global__ void gemm_reg_kernel_v6_2(float *dA, float *dB, float *dC, int M, int K, int N) {
...
template <int BM = 128, int BK = 8, int BN = 128, int TM = 8, int TN = 8>
float gemm_reg_v6_2(float *hA, float *hB, float *hC, int M, int K, int N) {
    float *dA, *dB, *dC;
    ...
    nvtxRangePush("gemm_reg_kernel_v6_2");
    gemm_reg_kernel_v6_2<BM, BK, BN, TM, TN><<<grid, block>>>(dA, dB, dC, M, K, N);
    cudaDeviceSynchronize();

    float avg_ms = 0.0f;
    {
        CudaTimer timer("gemm_reg_v6_2", BENCH_RUNS, print_on_destroy);
