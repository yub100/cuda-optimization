/* 
该版本基于v5_p,适配了不同BK，BK可以是8的整数倍（包括奇数倍）
结果显示边界判断带来的分支预测使得性能大大降低
*/
#include <iostream>
#include <iomanip>
#include <cstdlib>
...
__global__ void gemm_reg_kernel_v5_p(float *dA, float *dB, float *dC, int M, int K, int N) {
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        for (int i = 0; i < BM * BK; i += blockDim_x * blockDim_y * 4) {
            ...
            if (load_gmem_m < M && load_gmema_k + 3 < K) {
                FLOAT4(load_r[0]) = FLOAT4(dA[start_gmema]);
            } else {
                load_r[0] = load_gmema_k + 0 < K ? dA[start_gmema + 0] : 0.0f;
                load_r[1] = load_gmema_k + 1 < K ? dA[start_gmema + 1] : 0.0f;
                load_r[2] = load_gmema_k + 2 < K ? dA[start_gmema + 2] : 0.0f;
                load_r[3] = load_gmema_k + 3 < K ? dA[start_gmema + 3] : 0.0f;
            }
            // FLOAT4(load_r[0]) = FLOAT4(dA[start_gmema]);
            shared_A[load_smema_k][load_smema_m] = load_r[0];
            shared_A[load_smema_k + 1][load_smema_m] = load_r[1];
            shared_A[load_smema_k + 2][load_smema_m] = load_r[2];
            shared_A[load_smema_k + 3][load_smema_m] = load_r[3];
        ...
        for (int i = 0; i < BN * BK; i += blockDim_x * blockDim_y * 4) {
            ...
            if (load_gmemb_k < K && load_gmem_n + 3 < N) {
                FLOAT4(load_r[0]) = FLOAT4(dB[OFFSET(load_gmemb_k, load_gmem_n, N)]);
            } else {
                #pragma unroll
                for (int v = 0; v < 4; v++) {
                    load_r[v] = (load_gmemb_k < K && load_gmem_n + v < N)
                        ? dB[OFFSET(load_gmemb_k, load_gmem_n + v, N)]
                        : 0.0f;
                }
            }

            FLOAT4(shared_B[load_smemb_k][load_smemb_n]) = FLOAT4(load_r[0]);
